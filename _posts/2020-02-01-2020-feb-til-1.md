...
다음과 같은 내용을 build.gradle에 추가한다. `./gradlew jacocoTestReport`를 수행하면 xml형식의 리포트가 생성된다. 그 후 CI tool이 codecov를 지원한다면, `bash <(curl -s https://codecov.io/bash)`로 리포트를 업로드 할 수 있다.
